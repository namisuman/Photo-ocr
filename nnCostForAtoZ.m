function [J grad] = nnCostForAtoZ(nn_params,...
                                   input_layer_size,...
...
size(X);
size(Y);
J=-(sum(sum(Y.*log(X)+(1-Y).*log(1-X))))/m;
J=J+lambda/(2*m)*(sum(sum(Theta1'(2:end,:).^2))+sum(sum(Theta2'(2:end,:).^2))+sum(sum(Theta3'(2:end,:).^2)));
deltak=X-Y;
deltaj2=deltak*Theta3.*(B.*(1-B));
deltaj2=deltaj2(:,2:end);
deltaj=deltaj2*Theta2.*(A.*(1-A));
deltaj=deltaj(:,2:end);
Theta1_grad(:,2:end)=1/m*(deltaj'*x)(:,2:end)+lambda/m*Theta1(:,2:end);
Theta2_grad(:,2:end)=1/m*(deltaj2'*A)(:,2:end)+lambda/m*Theta2(:,2:end);
Theta3_grad(:,2:end)=1/m*(deltak'*B)(:,2:end)+lambda/m*Theta3(:,2:end);
Theta1_grad(:,1)=1/m*(deltaj'*x(:,1));
Theta2_grad(:,1)=1/m*(deltaj2'*A(:,1));
Theta3_grad(:,1)=1/m*(deltak'*B(:,1));
